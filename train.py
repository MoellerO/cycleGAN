...
def main():
    disc_H = Discriminator(in_channels=3).to(
        config.DEVICE)  # discrimator for horses
    disc_Z = Discriminator(in_channels=3).to(
        config.DEVICE)  # discrimator for zebras
    gen_Z = Generator(img_channels=3, num_residuals=9).to(
        config.DEVICE)  # generator Zebra
    gen_H = Generator(img_channels=3, num_residuals=9).to(
        config.DEVICE)  # generator horse
    opt_disc = optim.Adam(
